...
theorem P3.left : (A - B - C) ∧ (A - C - D) -> B - C - D := by
  ...
  have ConLintEC : C on cL.line ∩ EC := ⟨cL.mem C, ConEC⟩
  have LnparEC : cL.line ∦ EC := by
    by_contra! hNeg
    have emptyInter := Intersection.parallel_intersection_is_empty cL.line EC LneEC hNeg
    rw [emptyInter] at ConLintEC
    contradiction
  have LintECatC : cL.line intersects EC at C := (Intersection.single_point_of_intersection C cL.line EC ⟨LneEC, LnparEC⟩).mp ConLintEC
  ...
  · /- p113. (5) Then EC meets AB in a point between A and B (definition of "opposite sides" [ed. "splits" in our parlance]). -/
    ...
    have XinIntLine : X ∈ cL ∩ EC := by
      simp only [mem_inter_iff]; simp only at LintECatX;
      refine (mem_inter_iff X cL EC).mp ?_
      rw [LintECatX]
