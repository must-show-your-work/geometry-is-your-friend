...
atlas lemma 2.0.2 "Two distinct points on two lines force the lines to coincide"
  ...
  rcases via lemma 2.0.1 L M with LparM | LintMatX | LeqM
...
atlas lemma 2.0.4 "Segment A B is a subset of line A B"
  ...
  have h₂ : (ray A B : Line) ⊆ (line A B : Line) := via lemma 1.0.8
  intro P PonSeg
...
atlas lemma 2.0.5 "Line Points are Collinear"
  ...
  · rw [<- PeqA];
    apply (via lemma 1.0.7 B P).mpr
    by_cases suppose: B = P
    · rw [<- PeqA, suppose] at AneB; contradiction
    · exact via lemma 1.0.5 suppose
  · rw [<- PeqB]
    apply (via lemma 1.0.6 A P).mpr
    by_cases suppose: A = P
    · rw [<- PeqB, suppose] at AneB; contradiction
    · exact via lemma 1.0.5 suppose
  repeat exact Collinear.order_irrelevance (via axiom B.1 tween).collinear
...
atlas lemma 2.0.6 "Every point on extension A B is collinear with A and B"
  ...
  intro PonExtAB
  exact (via axiom B.1 PonExtAB.left).collinear
...
atlas lemma 2.0.7 "Every point on segment A B is collinear with A and B"
  ...
  intro PonSegAB
  apply via lemma 2.0.4 at PonSegAB
...
atlas lemma 2.0.8 "Ray Points are Collinear"
  ...
  intro PonAB
  apply via lemma 1.0.8 at PonAB
...
atlas lemma 2.0.9 "Segment A B and extension A B are disjoint"
  ...
  · intro P ⟨PonSeg, PonExt⟩
    ...
    · exfalso; exact via lemma 1.0.19 ⟨ABP, APB⟩
    · contradiction
    · contradiction
...
atlas lemma 2.0.10 "A ray A B is never equal to any line L"
  ...
  have ⟨X, colXAB, distinctXAB, XAB⟩ := via lemma 1.0.1 A B AneB
  separate at distinctXAB;
  have XonL : X on L := by
    idea "L = AB, and L = colXAB.line by the via lemma 2.0.2"
    have LeqXAB : L = colXAB.line := by
      have ABeqXAB := via lemma 2.0.2 AneB
  ...
  · rcases XonSeg with AXB | AeqX | BeqX
    · exact via lemma 1.0.18 ⟨XAB, AXB⟩
    · exact absurd AeqX XneA.symm
    · exact absurd BeqX XneB.symm
  · have ⟨ABX, _, _⟩ := XonExt
    exact via lemma 1.0.20 ⟨ABX, XAB⟩
...
atlas lemma 2.0.12 "A segment A B is never equal to any line L"
  ...
  have XonL : X on L := by
    have LeqXAB : L = colXAB.line :=
      via lemma 2.0.2 AneB ⟨AonL, colXAB.mem A, BonL, colXAB.mem B⟩
    rw [LeqXAB]; exact colXAB.mem X
  rw [<- ABeqL] at XonL
...
atlas lemma 2.0.14 "A point on a segment lies off the related extension"
  ...
  intro XonAB
  by_contra! hNeg
  have interEmpty : (segment A B : Line) ∩ (extension A B : Line) = ∅ := via lemma 2.0.13
...
atlas lemma 2.0.17 "Membership in the intersection of distinct nonparallel lines is the pointed intersection"
  ...
  · intro PinInter
    ...
    apply Line.eq_of_subset
    · intro Q QinInter
      have h := via lemma 2.0.16 L M LneM LnparM ⟨QinInter, PinInter⟩
      trivial
    · intro Q QisP
...
atlas lemma 2.0.18 "A line intersecting a segment intersects its containing ray at the same point"
  ...
  have XonSegAB : X on segment A B := via lemma 1.0.15 LintABatX
  have XonL : X on L := via lemma 1.0.14 LintABatX
  ...
  · push Not at counter
    ...
    · intro P PonLintRay
      ...
      have PeqX : P = X := via lemma 2.0.16 L (ray A B) LneRayAB LnparRayAB ⟨PonLintRay, XonLintRay⟩
      rw [PeqX]
      trivial
    · intro P PinSingleX
...
atlas lemma 2.0.20 "A line intersecting a ray intersects its containing line at the same point"
  ...
  intro LintRay
  have XonRayAB : X on ray A B := via lemma 1.0.15 LintRay
  have XonL : X on L := via lemma 1.0.14 LintRay
  have XABCol := @Line.«Ray Points are Collinear» A B X AneB XonRayAB
  have XonLineAB : X on line A B := via lemma 1.0.8 XonRayAB
  have XonRayAB : X on ray A B := by obvious
  have XinInter : X ∈ L ∩ line A B := by obvious
  have LnparRayAB : L ∦ ray A B := via lemma 2.0.19 LintRay
  have LnparLineAB : L ∦ line A B := by
    ...
    use X
    obvious
  have LneRayAB : L ≠ ray A B := Ne.symm (via lemma 2.0.10)
  have LneLineAB : L ≠ line A B := by
    ...
    have BonL : B on L := by
      ...
      rw [<- hNeg] at h; obvious
    have LintABatA : L intersects ray A B at A := (via lemma 2.0.17 A L (ray A B : Line) ⟨LneRayAB, LnparRayAB⟩).mp obvious
    have LintABatB : L intersects ray A B at B := (via lemma 2.0.17 B L (ray A B : Line) ⟨LneRayAB, LnparRayAB⟩).mp obvious
  ...
  · obtain ⟨P, LintABatP, PneX⟩ := counter
    have PinInter : P ∈ L ∩ line A B := by
      rw [LintABatP]
      trivial
    have PeqX : P = X := via lemma 2.0.16 L (line A B) LneLineAB LnparLineAB ⟨PinInter, XinInter⟩
    contradiction
  · push Not at counter
    apply Line.eq_of_subset
    · intro P PinInter
      exact counter P ((via lemma 2.0.17 P L (line A B) ⟨LneLineAB, LnparLineAB⟩).mp PinInter)
    · intro P PisX
      have PeqX : P = X := by obvious
      rw [PeqX]
      trivial
...
atlas lemma 2.0.21 "A line intersecting a segment intersects its containing line at the same point"
  ...
  intro LintSeg
  apply via lemma 2.0.18 at LintSeg
...
atlas lemma 2.0.22 "If A-X-B and L meets the segment at X then L splits A and B"
  ...
  push Not
  intro AoffL BoffL
  have distinctAXB := (via axiom B.1 AXB).distinct
  distinguish
  use X
  constructor
  · simp only [Segment.mem_def]; left; exact AXB
  · exact via lemma 1.0.14 LintAXBatX

atlas commentary := by
  via corollary 2.0.22
...
atlas corollary 2.0.22 "If L intersects segment A B at X, then L splits A and B"
  ...
  (L intersects (segment A B) at X) -> (L splits A and B) := by
    ...
    have hX : (A - X - B) ∨ A = X ∨ B = X := via lemma 1.0.15 LintABatX
    rcases hX with AXB | AeqX | BeqX
    · exact via lemma 2.0.22 AXB LintABatX
...
atlas lemma 2.0.24 "Crossing point of L through M between A and B forces A-X-B when M splits A B"
  ...
  have ⟨AoffM, BoffM⟩  : (A off M) ∧ (B off M) := by
    have hA := via lemma 2.0.23 AneX LintMatX
    have hB := via lemma 2.0.23 BneX LintMatX
    obvious
  ...
  have LeqAB : L = line A B := via lemma 2.0.2 AneB ⟨AonL, obvious, BonL, obvious⟩
  have PonL : P on L := by
    apply via lemma 2.0.4 at PonSeg
    rw [<- LeqAB] at PonSeg
    trivial
  have PeqX : P = X := by
...
atlas lemma 2.0.25 "Through X on L and E off L: L and line E X are distinct, nonparallel, meet at X"
  ...
  have int : L intersects (line E X) at X := (via lemma 2.0.17 X L (line E X) ⟨ne, npar⟩).mp XonLintEX
  obvious
...
atlas lemma 2.0.26 "A line crossing L at Z (not between A and B on L) guards A and B"
  ...
  · exact guard
  · exact absurd (via lemma 2.0.24 AneZ BneZ LintMatZ onL split) notAZB
...
atlas lemma 2.0.27 "The left outer point of a Between triple lies off the inner segment"
  ...
  intro hMem
  obtain ⟨distinctXYZ, _, _⟩ := via axiom B.1 h
  rcases hMem with YXZ | YeqX | ZeqX
  · exact via lemma 1.0.18 ⟨h, YXZ⟩
...
atlas lemma 3.0.1 "Bare intersection plus a shared point implies pointed intersection or coincidence"
  ...
  rcases via lemma 2.0.1 L M with empty | unique | equal
...
atlas lemma 3.0.2 "A line intersects itself (bare intersection of L with L)"
  : L intersects L := by
  obtain ⟨A, _, _, AonL, _⟩ := via axiom I.2 L
  exact ⟨A, AonL, AonL⟩
...
atlas corollary 3.7.3 "If L guards A and B, then L does not intersect segment A B"
  ...
  · subst AeqB
    ...
    · have : distinct A X A := (via axiom B.1 AXA).distinct
